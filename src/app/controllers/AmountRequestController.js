...
class AmountRequestController {
  async store(request,response){
    try {
      ...
      const next_end_point = await orderEndPoints(token,'amount_request')

      const amountRequestExists = await AmountRequest.findOne({where:{user_id:token}})
      
      if (amountRequestExists) throw new Error("Loan has already been made")

      await AmountRequest.create({id:v4(), user_id:token, loan_cents:data})
